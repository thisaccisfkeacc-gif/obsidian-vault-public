...
- **Root causes fixed in `PowerX.Core/Models/MacroItem.cs`:**
  1. Removed the `if (IsNew) return true;` short-circuit from `IsValid` (kept the `IsNew` property for legacy/recording compat, updated doc-comment). Empty newly-added blocks now show the warning dot immediately.
  2. Keyboard steps no longer hard-coded valid: `IsValid` for `MacroStepType.Keyboard` now requires a non-empty `Value` that isn't the `"None"` sentinel — matches past QA doc (`Block QA Findings.md`: *"warning dot shows when key is empty or 'None'"*).
- **Verified against Obsidian docs:** `IsNew` was intended as a *transient* suppression (testing-checklist: "IsNew block state (suppresses warning dot on new blocks)") — the clearing behavior described in the code comment never existed. Recording/optimization-created steps all carry real values, so they stay valid (no false dots).
- Build verified: 0 errors.
