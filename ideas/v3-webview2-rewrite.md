...
## Phases
1. **Interactive Dummy Prototype** — Build a clickable UI shell loaded in WebView2 (full modern layout, icons, dark mode, dummy data). No backend wires attached yet so you can double-click, test the look, and request UI changes.
2. **UI Design Approval** — Fine-tune colors, icons, and layout based on your feedback until the prototype is 100% approved.
3. **Bridge & Backend** — Wire up C# backend services to the JS frontend (macro CRUD, hotkeys, settings).
4. **Features** — Port all V2 features one by one into the wired UI.
5. **Polish & Performance** — Suspend/resume tuning and tray optimization.
6. **Replace** — V3 becomes the main app.
