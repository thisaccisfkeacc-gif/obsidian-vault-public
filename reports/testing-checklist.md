...
| #   | Test                                                | Status | Notes                                          |
| --- | --------------------------------------------------- | ------ | ---------------------------------------------- |
| 35  | Custom Keystroke fires the correct key combo        | `[ ]`  | bug                                            |
| 36  | App Switcher switches to the correct app            | `[ ]`  | bug                                            |
| 38  | Press Enter after editing name → saves              | `[ ]`  |                                                |
| 39  | Press Escape during name edit → cancels             | `[ ]`  | it saves it actually  instead of canceling     |
| 40  | Enable/disable toggle on custom action card         | `[ ]`  | not checked/ doesnt have this feature actually |
| 41  | Cycle Through Windows setting works on App Switcher | `[ ]`  | not checked                                    |
| 411 | Add Custom Macro action                             | `[ ]`  | not checked                                    |
| 414 | App Switcher: Window Action (Activate vs Close)     | `[ ]`  | doesnt have this feature                       |
| 415 | App Switcher: Minimize if Active                    | `[ ]`  | doesnt work properly                           |
| 416 | App Switcher: Strict Instance Tracking              | `[ ]`  | not checked/doesnt work properly               |
| 417 | App Switcher: Restore Window Size                   | `[ ]`  | not checked                                    |
...
| #   | Test                                                       | Status | Notes                                                                            |
| --- | ---------------------------------------------------------- | ------ | -------------------------------------------------------------------------------- |
| 56  | Trigger mode: Schedule (Interval)                          | `[ ]`  | i think that it doesnt exist coz feature like this is not here                   |
| 57  | Trigger mode: Mobile Remote (visible on phone)             | `[ ]`  |                                                                                  |
| 58  | Trigger mode: Auto-Detect (Screen Event)                   | `[ ]`  | not able to capure ui element, and needs fixing of gear icon for image and pixel |
